...
export default function AdminPage() {
  ...
  const fetchUsers = async () => {
          const res = await axios.get('https://fe-bank-1.onrender.com/admin/users', getAuthHeader());
  ...
  const handleSave = async e => {
    ...
    try {
      if (editUser) {
        ...
        await axios.put(`https://fe-bank-1.onrender.com/admin/users/${editUser._id}`, { email, role, isActive }, getAuthHeader());
        ...
        await axios.post('https://fe-bank-1.onrender.com/admin/users', { username, email, role, isActive, password }, getAuthHeader());
        setMsg('Đã thêm user!');
      }
      closePopup();
      fetchUsers();
  ...
  const handleDelete = async id => {
    if (!window.confirm('Bạn có chắc muốn xóa user này?')) return;
            await axios.delete(`https://fe-bank-1.onrender.com/admin/users/${id}`, getAuthHeader());
    setMsg('Đã xóa user!');
    fetchUsers();
  };
  ...
  const handleRoleChange = async (userId, newRole) => {
    try {
      const user = users.find(u => u._id === userId);
              await axios.put(`https://fe-bank-1.onrender.com/admin/users/${userId}`, { email: user.email, role: newRole, isActive: user.isActive }, getAuthHeader());
  ...
  const handleStatusChange = async (userId, isActive) => {
    try {
      ...
      const res = await axios.put(
        `https://fe-bank-1.onrender.com/admin/users/${userId}`,
